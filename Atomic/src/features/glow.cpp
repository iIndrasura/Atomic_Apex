...
// Glow 2 bool
// Bool to enable glow
bool Glow::shouldGlow2Enable(rx_handle process) {
    if ( ConfigValues::GLOW_ENABLED2 == 1 /* && (levelClass.isPlayable(process) || levelClass.isSpecialMode(process)) */ )
		 {
        return true;
    }
    return false;
...
// GLOW 2
void Glow::ActivateGlow2(rx_handle process, QWORD ClientEntityList, QWORD Sensitivity, int GameMode ) 
{
    if (shouldGlow2Enable(process))
    {   
        QWORD localplayer = localplayerClass.getLocalplayerPointer(process);
